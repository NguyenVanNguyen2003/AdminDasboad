...
export default function ProductsPage() {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = bills.map((bill) => bill.car);
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/get-bill')
      .then((response) => {
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, car, user, phone, email, rentStartDate, rentEndDate, totalAmount, isPaid } = row;
                    const selectedUser = selected.indexOf(car) !== -1;

                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, <TableCell align="left">{totalAmount}</TableCell>
                          )} />
                        </TableCell>
                        ...
                        <TableCell align="left">{rentStartDate}</TableCell>
                        <TableCell align="left">{rentEndDate}</TableCell>
